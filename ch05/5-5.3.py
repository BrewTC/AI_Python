...
def get_sqa(f):           # f 為已開啟的檔案物件
    ...
    for line in f:  # 走訪檔案中的每一行字串 (可能是敘述或問題)
        ...
        if '\t' in line:  # 如果字串中有 Tab 字元, 表示其為問題 (而不是敘述)
            ...
            tsup = map(lambda x: int(x)-1, tsup.split()) #切割 tsup 中的數字並轉為整數索引
            tsup = list(tsup)  # 先將 map() 傳回的物件轉成 list
            ts2 = [i for i in range(len(ts)) if i not in tsup and ts[i]]
                                                #↑只取不在 tsup 且 ts[i] 不是空字串的索引值
            from numpy.random import randint
            if addn > 0:  # 大於 0 才要加入無關的敘述
                while len(ts2) > addn:  # 當元素數量超過 addn 時就進行迴圈
                    ts2.pop(randint(len(ts2)))  # 隨機刪除串列中的一個元素
                tsup = sorted(ts2 + tsup)   # 將無關敘述和支持敘述的索引串接起來, 然後排序

            sup = ''      # 用來串接支持答案的敘述
            for i in tsup:      # 將無關及支持敘述的字串都串接成一個字串
                sup += ts[i]   # 將每個無關及支持的敘述串接起來
...
#           'Training & Validation Acc', ('Epoch','Acc'))
    ...
    print(f'評量成效：loss / Acc = {loss:.4f} / {acc:.4f}')
...
for v in vers:
    for n in (1, 2, 3):
        addn = n     # 要加多少個無關的敘述
        print(f'●使用 {v} 版本訓練, 故事樣本中加 {n} 個無關的敘述')
        for i in (1,2,3,4,5,11,12,13,14,15,16,20):
            run_qa(path, v, i, verbose=0)
